...
#: transposing to (freq, 2, 2) - see VNACalibSlopeCustomMeas._export_h5
_MATRIX_POSITION = {
    "Sig1Sig1": (0, 0),
    "Sig1Sig2": (0, 1),
    "Sig2Sig1": (1, 0),
...
class VNACalibSlopeCustomMeas(BaseMeasurement):
    ...
    def execute(self):
        ...
        self.vna.power_slope(power_slope)

        labels = tuple(_MATRIX_POSITION)
        ...
        with safe_run(self.instruments):
            ...
            for n_pt, bw in zip(n_pts_list, bw_list):
                ...
                trace_data = {
                    label: self.vna.read_raw_data(label) for label in labels
                }

                self._run_one_sweep(
                    experiment, station, run_metadata, n_pt, bw, freq_data, trace_data
                )
                self._export_h5(n_pt, freq_data, trace_data)
                bar.update()
            bar.refresh()

    def _run_one_sweep(
        self, experiment, station, run_metadata, n_pt, bw, freq_data, trace_data
    ):
        ...
        meas = Measurement(
            name="vna_calib_slope_custom_meas", exp=experiment, station=station
        )
    ...
    def _export_h5(self, n_pt, freq_data, trace_data):
        """Same `.h5` layout `VNACalibSlopeCustomMeas.execute` used to
        write directly. Written straight from the values just measured
        (the same ones passed to `datasaver.add_result` in
        `_run_one_sweep`) rather than reading them back from the db -
        each `(pts, bw)` acquisition is a single atomic VNA sweep with no
        useful mid-point to checkpoint, so there's nothing a db round-trip
        would add here (unlike the two whole-sweep-is-one-run scripts,
        where writing point-by-point during the loop is what protects
        against a mid-sweep crash)."""
